...
import { Phone, FileText, Calendar, Building, Briefcase, Home, Euro, Settings, TrendingDown, CreditCard, Clock, Star, User } from 'lucide-react';
...
const AgencyDashboard = ({ personalOnly = false }: AgencyDashboardProps) => {
  const { profile: currentProfile } = useAuth();
  const [period, setPeriod] = useState<'week' | 'month' | 'year'>('month');
  const [showTargetsDialog, setShowTargetsDialog] = useState(false);
  const userSede = currentProfile?.sede || 'CITTÀ DI CASTELLO';

  // When personalOnly, useKPIs still works — it aggregates from useDailyData
  // which for agents only returns their own data (allData is disabled for agents)
  ...
  return (
    <div className="px-4 pb-8 space-y-4 animate-fade-in">
      ...
      <div className="bg-card rounded-2xl border border-border p-4">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-12 h-12 rounded-xl bg-foreground text-background flex items-center justify-center">
            {personalOnly ? <User className="w-6 h-6" /> : <Star className="w-6 h-6" />}
          </div>
          <div className="flex-1">
            <h2 className="tracking-wide uppercase text-xl font-medium">
              {personalOnly ? (currentProfile?.full_name || 'I miei numeri') : userSede}
            </h2>
      ...
      <p className="text-[10px] font-medium tracking-[0.15em] uppercase text-muted-foreground px-1">
        {personalOnly ? 'LA MIA PERFORMANCE' : 'PERFORMANCE GENERALE (SOMMA AGENTI)'}
      </p>
      ...
      {!personalOnly && <SedeTargetsDialog open={showTargetsDialog} onOpenChange={setShowTargetsDialog} />}
    </div>
  );
};
