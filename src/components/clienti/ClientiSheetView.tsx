...
type ColumnDef = {
  ...
  minWidth: number;
  editable?: boolean;
  type?: 'text' | 'number' | 'date' | 'select' | 'status' | 'agent' | 'regions' | 'types';
};

const COLUMNS: ColumnDef[] = [
  { key: 'paese', label: 'Country', width: 100, minWidth: 70, editable: true, type: 'text' },
  { key: 'nome', label: 'Name', width: 160, minWidth: 100, editable: true, type: 'text' },
  { key: 'email', label: 'Email', width: 200, minWidth: 120, editable: true, type: 'text' },
  { key: 'telefono', label: 'Phone', width: 150, minWidth: 100, editable: true, type: 'text' },
  ...
  { key: 'data_submission', label: 'Date', width: 110, minWidth: 80, editable: false, type: 'date' },
  { key: 'last_contact_date', label: 'Last Contact', width: 120, minWidth: 80, editable: false, type: 'date' },
...
export function ClientiSheetView({ clienti, agents, onCardClick, onUpdate, searchQuery }: ClientiSheetViewProps) {
  ...
  const getCellValue = useCallback((cliente: Cliente, col: ColumnDef): string => {
    switch (col.key) {
      ...
      <div style={{ minWidth: totalWidth }}>
        ...
        <div className="flex sticky top-0 z-10 bg-muted/80 backdrop-blur-sm border-b">
          ...
          {COLUMNS.map(col => (
            <div
              key={col.key}
              className="relative flex items-center border-r text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-2 py-2 select-none"
        ...
        {filtered.map((cliente, idx) => (
          ...
          >
            ...
            {COLUMNS.map(col => (
              ...
              >
                <EditableCell
                  value={getCellValue(cliente, col)}
